...
void ReportGenerator::printStats(const Graph &g) const {
  ...
  std::cout << "  - Total de Vértices (Lemas Únicos): " << vertices.size()
            << std::endl;
  ...
  std::cout << "  - Total de Arestas Ponderadas: " << (totalEdges / 2)
            << std::endl;
}

void ReportGenerator::printTopTerms(const Graph &g, int k) const {
  std::cout << "\n[2] TERMOS MAIS RELEVANTES (MaxHeap - Top " << k << ")"
            << std::endl;
  MaxHeap heap;
  heap.buildFromGraph(g);
  auto topK = heap.getTopK(k);
  for (size_t i = 0; i < topK.size(); ++i) {
    std::cout << "  " << (i + 1) << ". \"" << topK[i].word
              << "\" (Score de Conexão: " << topK[i].score << ")" << std::endl;
  }
}

void ReportGenerator::printBFSDistances(const Graph &g,
                                        const std::string &rootTerm) const {
  if (g.hasVertex(rootTerm)) {
    std::cout << "\n[3] DISTÂNCIA DE TERMOS A PARTIR DE \"" << rootTerm
              << "\" (BFS)" << std::endl;
    BFS bfs;
    auto bfsResult = bfs.traverse(g, rootTerm);
    // Agrupa e mostra termos até nível 3
    for (int lvl = 1; lvl <= 3; ++lvl) {
      std::cout << "  - Nível " << lvl << " (Conexão direta ou a " << lvl
                << " saltos):" << std::endl;
      bool found = false;
...
void ReportGenerator::printShortestPath(const Graph &g,
  ...
  if (g.hasVertex(startWord) && g.hasVertex(endWord)) {
    ...
    std::cout << "  Calculando caminho forte de \"" << startWord << "\" até \""
              << endWord << "\"..." << std::endl;
    ...
    if (pathRes.found) {
      ...
      for (size_t i = 0; i < pathRes.path.size(); ++i) {
        ...
        if (i + 1 < pathRes.path.size())
          std::cout << " -> ";
      }
      std::cout << "\n  - Peso total acumulado: " << pathRes.totalWeight
                << std::endl;
    } else {
      std::cout
          << "  - Nenhum caminho de conexão encontrado entre esses termos."
          << std::endl;
    }
  } else {
    std::cout << "\n[4] MENOR CAMINHO DE ASSOCIAÇÃO (Dijkstra)" << std::endl;
    std::cout << "  - Erro: Um ou ambos os termos (\"" << startWord << "\", \""
              << endWord << "\") não existem no grafo." << std::endl;
  }
}

void ReportGenerator::exportGraphToJson(const Graph &g,
...
void ReportGenerator::generateSummary(const Graph &g,
  ...
  std::cout << "  - Arquivo '" << jsonPath << "' exportado com sucesso."
            << std::endl;
  std::cout << "  - Digite: python3 data/visualize.py para abrir o grafo "
